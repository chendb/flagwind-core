...
public class EntityFieldUtils {
    ...
    private static final EntityFieldParser parser;

    static {
        String version = System.getProperty("java.version");
        if (version.contains("1.7.")) {
            parser = new Jdk6_7FieldParser();
        } else {
            parser = new Jdk8FieldParser();
        }
    }
    ...
    public static List<EntityField> getFields(Class<?> entityClass) {
        return parser.getFields(entityClass);
    }
    ...
    public static List<EntityField> getProperties(Class<?> entityClass) {
        return parser.getProperties(entityClass);
    }
    ...
    public static List<EntityField> getAll(Class<?> entityClass) {
        List<EntityField> fields = parser.getFields(entityClass);
        List<EntityField> properties = parser.getProperties(entityClass);
        // 拼到一起，名字相同的合并
     ...
     * EntityField转换接口
     */
    static interface EntityFieldParser {
        /**
         * 获取全部的Field
         *
         * @param entityClass
     ...
     * 支持jdk8
     */
    static class Jdk8FieldParser implements EntityFieldParser {
        /**
         * 获取全部的Field
         *
         * @param entityClass
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                if (!Modifier.isStatic(field.getModifiers())) {
                    if (level.intValue() != 0) {
                        // 将父类的字段放在前面
                        fieldList.add(index,
                                new EntityField(field, getPropertyDescriptor(entityClass, field.getName())));
                        index++;
                    } else {
     ...
     * 支持jdk6,7
     */
    static class Jdk6_7FieldParser implements EntityFieldParser {
        ...
        public List<EntityField> getProperties(Class<?> entityClass) {
            ...
            for (PropertyDescriptor desc : descriptors) {
                if (desc != null && !"class".equals(desc.getName())) {
                    ...
                    if (desc.getReadMethod() != null && desc.getReadMethod().getGenericReturnType() != null
                            && desc.getReadMethod().getGenericReturnType() instanceof TypeVariable) {
                        entityField.setJavaType(genericMap
                                .get(((TypeVariable<?>) desc.getReadMethod().getGenericReturnType()).getName()));
                        ...
                        entityField.setJavaType(genericMap.get(
                                ((TypeVariable<?>) desc.getWriteMethod().getGenericParameterTypes()[0]).getName()));
                    }
                ...
                if (!Modifier.isStatic(field.getModifiers()) && !Modifier.isTransient(field.getModifiers())) {
                    ...
                    if (field.getGenericType() != null && field.getGenericType() instanceof TypeVariable) {
                        if (genericMap == null
                            ...
                            entityField
                                    .setJavaType(genericMap.get(((TypeVariable<?>) field.getGenericType()).getName()));
                        }
                    } else {
